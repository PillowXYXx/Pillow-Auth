...
def verify_key():
    ...
    if status == "unused":
        ...
        c.execute("UPDATE licenses SET status='used', hwid=?, device_name=?, expires_at=?, redeemed_at=? WHERE key_code=?", 
                  (hwid, device_name, new_expires_at, redeemed_time, key))
        conn.commit()
        conn.close()
        ...
        if stored_hwid == hwid:
            # Increment run count
            c.execute("UPDATE licenses SET run_count = run_count + 1 WHERE key_code=?", (key,))
            conn.commit()
            conn.close()
...
def link_discord():
    ...
    # (Optional: Allow re-linking? Let's assume yes for now, or strict no?)
    # Let's update it.
...
def get_user_keys():
    ...
    c = conn.cursor()
    c.execute("SELECT key_code, status, duration_hours, expires_at, device_name, hwid, run_count FROM licenses WHERE discord_id=?", (discord_id,))
    rows = c.fetchall()
...
def list_keys():
    ...
    c.execute("SELECT key_code, status, device_name, hwid, created_at, duration_hours, expires_at, note, discord_id, run_count FROM licenses ORDER BY created_at DESC")
    rows = c.fetchall()
